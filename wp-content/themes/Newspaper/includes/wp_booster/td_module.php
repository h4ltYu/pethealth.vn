        ...
        if ($this->is_review and td_util::get_option('tds_m_show_review') != 'hide') {
            ...
            if (td_util::get_option('tds_m_show_date') != 'hide') {
                ...
                if ($modified_date == 'yes' || td_util::get_option('tds_m_show_modified_date') == 'yes') {
                    $buffy .= '<time class="entry-date updated td-module-date' . $visibility_class . '" datetime="' . date(DATE_W3C, $td_article_date_unix) . '" >' . get_the_modified_date(get_option( 'date_format' ), $this->post->ID) . '</time>';
                }
                else {
