...
//td_demo_state::update_state("video_pro", 'full');
...
function hide_external() {
    remove_action('admin_bar_menu', 'action_network_admin_menu',95);
    remove_menu_page('wpel-settings-page');
     remove_menu_page('td_theme_welcome');
}
// add_action( 'admin_init', 'hide_external');

add_filter( 'gettext', 'nef_translate_woocommerce_strings', 999 );

function nef_translate_woocommerce_strings( $translated ) {

    // Từ thứ 1
    $translated = str_ireplace( 'Read more', 'Xem thêm', $translated );
    return $translated;
}
?>
